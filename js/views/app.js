...
define(['jquery', 'underscore', 'backbone', 'models/record', 'models/recordcontainer', 'collections/recordcontainers', 'views/record', 'common', 'eventmgr'], function($, _, Backbone, Record, RecordContainer, RecordContainers, RecordView, Common, eventManager) {
  'use strict';
  return Backbone.View.extend({
    ...
    initialize: function() {
      ...
      this.records = this.recordContainers.records;
      calculateWarp = _.debounce(this.calculateRender, 5, false);
      this.recordContainers.fetch().done(function() {
        return eventManager.trigger('reccntrsready');
      });
      this.listenTo(this.records, 'add', this.renderOne);
      this.listenTo(this.records, 'all', calculateWarp);
      return this.listenTo(this.records, 'reset', this.render);
    },
    render: function() {
      var _this = this;
